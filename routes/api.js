...
router.get("/configuration/locations", function (req, res, next) {
    API.configuration.locations.locations(req.session.xapi, devAccount, function (err, response, request, body) {
        if (err) res.status(err.status).send({ error: err, request: request, body: body });
        else res.json({ response: response, request: request, body: body });
    })
})
router.get("/configuration/ssids", function (req, res, next) {
    API.configuration.ssids.get(req.session.xapi, devAccount, function (err, response, request, body) {
        if (err) res.status(err.status).send(err);
        else res.json({ response: response, request: request, body: body });
    })
})
router.get("/configuration/webhooks", function (req, res, next) {
    API.configuration.webhooks.get(req.session.xapi, devAccount, function (err, response, request, body) {
        if (err) res.status(err.status).send(err);
        else res.json({ response: response, request: request, body: body });
    })
})
function checkWebhook(req, callback) {
    API.configuration.webhooks.get(req.session.xapi, devAccount, function (err, response, request, body) {
        if (err) callback(err, null);
            ...
            console.log(request);
            console.log(body);
            callback(null, webhook, request, body);
        }
    })
}
router.post("/configuration/webhooks", function (req, res, next) {
    if (req.session.xapi.ownerId) {
        ...
        API.configuration.webhooks.create(req.session.xapi, devAccount, subscription, function (err, response, request, body) {
            if (err) {
                ...
                req.session.save();
                res.json({ response: response, request: request, body: body });
            }
        })
...
router.delete("/configuration/webhooks", function (req, res, next) {
    if (req.session.webhookId) {
        ...
        API.configuration.webhooks.remove(req.session.xapi, devAccount, req.session.webhookId, function (err, response, request, body) {
            if (err) res.status(err.status).send(err);
            else res.json({ response: response, request: request, body: body });
        })
...
router.get("/identity/credentials", function (req, res, next) {
    API.identity.credentials.getCredentials(req.session.xapi, devAccount, null, null, null, null, null, null, null, null, null, null, null, null, function (err, response, request, body) {
        if (err) res.status(err.status).send(err);
        else res.json({ response: response, request: request, body: body });
    })
})
router.get("/identity/userGroups", function (req, res, next) {
    API.identity.userGroups.getUserGroups(req.session.xapi, devAccount, null, null, function (err, response, request, body) {
        if (err) res.status(err.status).send(err);
        else res.json({ response: response, request: request, body: body });
    })
})
...
router.get("/location/clients", function (req, res, next) {
    API.configuration.ssid.GET(req.session.xapi, devAccount, function (err, response, request, body) {
        if (err) res.status(err.status).send(err);
        else res.json({ response: response, request: request, body: body });
    })
})
...
router.get("/monitor/clients", function (req, res, next) {
    API.monitor.clients.list(req.session.xapi, devAccount, function (err, response, request, body) {
        if (err) res.status(err.status).send(err);
        else res.json({ response: response, request: request, body: body });
    })
})
router.get("/monitor/devices", function (req, res, next) {
    API.monitor.devices.list(req.session.xapi, devAccount, function (err, response, request, body) {
        if (err) res.status(err.status).send(err);
        else res.json({ response: response, request: request, body: body });
    })
})
...
router.get("/presence/clientcount", function (req, res, next) {
    if (req.query.locationId) {
        ...
        API.clientlocation.clientcount(req.session.xapi, devAccount, locationId, startTime, endTime, function (err, response, request, body) {
            if (err) res.status(err.status).send(err);
            else res.json({ response: response, request: request, body: body });
        })
...
router.get("/presence/clientpresence", function (req, res, next) {
    if (req.query.locationId) {
        ...
        API.clientlocation.clientpresence(req.session.xapi, devAccount, locationId, startTime, endTime, timeUnit, function (err, response, request, body) {
            if (err) res.status(err.status).send(err);
            else res.json({ response: response, request: request, body: body });
        })
    } else res.status(401).send("Error: no locationId");
...
router.get("/presence/clienttimeseries", function (req, res, next) {
    if (req.query.locationId) {
        ...
        var startTime = new Date(new Date().setDate(new Date().getDate() - 7)).toISOString();
        var timeUnit = "OneHour";
        API.clientlocation.clienttimeseries(req.session.xapi, devAccount, locationId, startTime, endTime, timeUnit, function (err, response, request, body) {
            if (err) res.status(err.status).send(err);
            else res.json({ response: response, request: request, body: body });
        })
    } else res.status(401).send("Error: no locationId");
...
router.get("/presence/waypoints", function (req, res, next) {
    if (req.query.locationId) {
        ...
        var endTime = new Date().toISOString();
        var startTime = new Date(new Date().setDate(new Date().getDate() - 7)).toISOString();
        API.clientlocation.waypoints(req.session.xapi, devAccount, locationId, startTime, endTime, function (err, response, request, body) {
            if (err) res.status(err.status).send(err);
            else res.json({ response: response, request: request, body: body });
        })
    } else res.status(401).send("Error: no locationId");
